...
def execute(gpu, exp_batch, exp_alias, suppress_output=True, number_of_workers=12):
    ...
    try:
        ...
        if g_conf.OPTIMIZER == 'Adam':
            ...
            optimizer = optim.SGD(model.parameters(), lr=g_conf.LEARNING_RATE, momentum=0.9)
        else:
            raise ValueError
        ...
        for data in data_loader:
            ...
            if g_conf.ACTION_CORRELATION_MODEL_TYPE is not None:
                reweight_params = {'importance_sampling_softmax_temper': g_conf.SOFTMAX_TEMPER,
                                   'importance_sampling_threshold': dataset.get_action_predict_loss_threshold(g_conf.THRESHOLD_RATIO),
                                   'importance_sampling_threshold_weight': g_conf.THRESHOLD_WEIGHT,
                                   'action_predict_loss': data['action_predict_loss'].cuda().squeeze()
                                   }
            else:
                reweight_params = {}
            ...
            loss_function_params = {
                ...
                'importance_sampling_method': g_conf.IMPORTANCE_SAMPLE_METHOD,
                **reweight_params,
