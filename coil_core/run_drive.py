...
def driving_benchmark(checkpoint_number, gpu, town_name, experiment_set, exp_batch, exp_alias,
                      params, control_filename, task_list, prefix=None):
    """
    ...
    Args:
        ...
        prefix: the prefix of checkpoint name: which is decided by finetune or not
    Returns:
    ...
    try:
        ...
        checkpoint_name = prefix + str(checkpoint_number)

        checkpoint = torch.load(os.path.join('_logs', exp_batch, exp_alias
                                             , 'checkpoints', checkpoint_name + '.pth'))

        coil_agent = CoILAgent(checkpoint, town_name)
        print ("Checkpoint ", checkpoint_name)
        coil_logger.add_message('Iterating', {"Checkpoint": checkpoint_name}, checkpoint_number)

        """ MAIN PART, RUN THE DRIVING BENCHMARK """
        run_driving_benchmark(coil_agent, experiment_set, town_name,
                              exp_batch + '_' + exp_alias + '_' + checkpoint_name
                              + '_drive_' + control_filename
                              , True, params['save_images'], params['save_videos'], params['save_processed_videos'],
                              params['policy_roll_out'], params['host'], port)

        """ Processing the results to write a summary"""
        path = exp_batch + '_' + exp_alias + '_' + checkpoint_name \
