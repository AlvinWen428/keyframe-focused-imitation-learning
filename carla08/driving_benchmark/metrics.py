...
class Metrics(object):
    ...
    def compute(self, path):
        ...
        metrics_dictionary = {'episodes_completion': {w: [0] * len(tasks) for w in all_weathers},
                              ...
                              'path_completion': {w: [[] for i in range(len(tasks))] for w in all_weathers},
                              'time_out': {w: [0] * len(tasks) for w in all_weathers}
                              }
        ...
        for t in range(len(tasks)):
            ...
            for w in weathers:
                ...
                metrics_dictionary['time_out'][w][t] = \
                    ((experiment_results_matrix[:, header.index('final_time')] -
                      experiment_results_matrix[:, header.index('time_out')]) == 0).astype(int).tolist()
                ...
                for episode_experiment_metrics in episode_experiment_metrics_matrix:
                    ...
                    metrics_dictionary['average_speed'][w][t] = \
                        km_run_episodes / (experiment_results_matrix[count,
                                                                     header.index(
                                                                         'final_time')] / 3600.0)
                    metrics_dictionary['path_completion'][w][t].append(
                        km_run_episodes*1000 / experiment_results_matrix[count, header.index('initial_path_distance')])
