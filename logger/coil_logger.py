...
def recover_loss_window(dataset_name, iteration):
    ...
    if not os.path.exists(file_name):
        return []
    # fixed the bug if the file has only 1 line, the loaded array will be a 0-d array.
...
def add_image(tag, images, iteration=None):
    ...
    if iteration is not None:
        if iteration % IMAGE_LOG_FREQUENCY == 0:
            ...
            # if the input is a stack of frames, we save the current frame, i.e. the last one.
            images = images[:, -3:, :, :]
